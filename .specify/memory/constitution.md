...
Change: TWO edits to the v2.0.0 constitution:
  1. One-time repositioning statement added to the preamble: Prompting Press expands its thesis
     from "minimal, validation-blind core" to "a minimal, validation-blind core PLUS earned,
     opt-in seams," motivated by a real second consumer (Bellwether). This is the shared anchor
     that specs 018/019 cite.
  2. A redefinition of spec-008 FR-017(b): the shipped FR-017(b) mandated that the only way to
     vary a prompt is a copy-with-overlay that "shallow-replaces each supplied top-level field
     (no deep merge)". Introducing MergeStrategy::Merge (top-level key union of variables/
     variants/metadata) is a BACKWARD-INCOMPATIBLE redefinition of that FR — analogous to how
     spec-015 redefined spec-002's guard body-invariant. This drives the MAJOR bump. Recorded
     in DECISIONS.md.
  Plus a Principle VI additive clarification (derive gains a merge-strategy axis; compile-time
  vs. runtime coverage asymmetry preserved). The clarification is MINOR on its own, but the
  FR-017(b) redefinition makes the overall change MAJOR.
Migration: callers using derive() with no strategy argument see NO behavior change (SC-002);
  Replace is the default. Callers that relied on "the only way to vary a prompt is wholesale
  replace" must update if they depended on that guarantee being absolute.
Templates checked:
...
> **v3.0.0 Repositioning (spec 017, 2026-07-08):** Prompting Press began as a minimal,
> validation-blind core that never grows into a framework. Having earned a real second consumer
> (Bellwether) that needs earned, opt-in derivation seams, the thesis is extended to: a
> **minimal, validation-blind core PLUS earned, opt-in seams** — seams that are justified by
> two concrete consumers, not anticipated generality. Every new seam still requires a second
> consumer to exist before it ships (Scope Discipline). Specs 018 and 019 cite this statement
> as their shared constitutional anchor.
...
- **`derive` gains a merge-strategy axis** (spec 017). `Prompt.derive` accepts an optional
  `MergeStrategy` (enum in all three bindings) that selects whether map-typed fields
  (`variables`, `variants`, `metadata`) union top-level keys (child-wins, `Merge`) or replace
  wholesale (`Replace`, the default — non-breaking). The `validation_required` coverage
  asymmetry from the spec-008 amendment is **preserved**: Python/TS raise/throw against the
  merged variable set at construction; Rust guarantees coverage at compile time via the generic
  validator type; Rust MUST NOT add a runtime coverage throw. The union algorithm is implemented
  **once** in the consumer crate (`merge_definitions`) and called by both the typed Rust path
  and the Node binding (Principle I / FR-018). This supersedes spec-008 FR-017(b)'s "no deep
  merge / wholesale-replace only" wording — see `DECISIONS.md`. (Adopted with spec 017.)
