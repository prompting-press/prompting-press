...
let vars = Ask { topic: "rivers".into() };
```
</TabItem>
...
let bad = GuardConfig { enabled: true, advisory: Some("Missing the required markers.".into()) };
...
```rust
let messages = vec![
    ("system", result.guard.unwrap_or_default()),
    ("user",   result.text),
];
```
</TabItem>
...
<TabItem label="Rust">
```rust
let mut request_messages = vec![("system", result.guard.unwrap_or_default())];
for m in comp.resolve()? {
    request_messages.push((m.role.as_str(), m.text));
}
```
</TabItem>
<TabItem label="Python">
...
const bodyMessages = comp.resolve().map((m) => ({ role: m.role, content: m.text }));
