...
The merge strategy for [`Prompt.derive`]. - `REPLACE` (default) — each overlay-present field replaces the base's field wholesale. Byte-identical to the pre-017 behavior.
- `MERGE` — map-typed fields (`variables`, `variants`, `metadata`) union at top-level keys (child-wins-whole-entry, no recursion). Scalar fields replace when overlay-present. Pass as a keyword argument: `prompt.derive(overlay, strategy=MergeStrategy.MERGE)`.
...
```python
derive(self, /, overlay, *, validators=None, strategy=None)
```
...
consumer's `Prompt::derive_with` (full re-validation: agreement, parse, reserved name). `strategy` selects how map-typed fields (`variables`, `variants`, `metadata`) combine:
- `MergeStrategy.REPLACE` (default) — overlay replaces each present field wholesale.
- `MergeStrategy.MERGE` — union top-level keys (child-wins, no recursion); scalars replace. Validators carry forward from `self` by default. Pass `validators=SomeModel` to
override or augment the bound validator on the derived prompt. Coverage is re-checked
against the merged variable set. The original `Prompt` is untouched (immutability guaranteed). # Errors - [`PromptValidationError`](crate::error::PromptValidationError) — the merged definition violates any construction invariant, or the (carried/supplied) validators do not cover a `validation_required` variable in the merged definition.
...
```python
provenance_attributes(self, /)
```

Return the four provenance fields as a flat `prompting_press.prompt.*` attribute map. Returns a `dict[str, str]` containing exactly four entries keyed by the library-owned
`prompting_press.prompt.*` constants. Suitable for direct use as telemetry span
attributes (e.g. `span.set_attributes(result.provenance_attributes)`). The map is an explicit allowlist — it NEVER includes `text`, `guard`,
`output_model`, or any other field. It requires no telemetry dependency. Keys are NOT `OTel` `gen_ai.*` keys; a consumer may remap them.
...
```python
class FileSystemLoader(base: 'str | pathlib.Path', suffix: 'str' = '.yaml', max_bytes: 'int' = 1048576) -> 'None'
```

Load prompt files from a configured base directory. Maps a logical key to ``&#123;base&#125;/&#123;key&#125;&#123;suffix&#125;`` and returns the file's raw text. Parameters
----------
base: The directory under which prompt files live. Canonicalized at construction.
suffix: File name suffix appended to every key (default ``".yaml"``).
max_bytes: Maximum file size in bytes (default 1 MiB). A file exceeding this limit raises ``PromptLoadError`` with code ``load_io``. Raises
------
PromptLoadError ``load_not_found`` if ``base`` does not exist; ``load_io`` on other OS errors.

#### `DEFAULT_MAX_BYTES`

```python
DEFAULT_MAX_BYTES(self)
```
...
```python
load(self, key: 'str') -> 'str'
```

Load the prompt source for ``key``. Parameters
----------
key: Logical identifier; treated as a relative path under :attr:`base`. Returns
-------
str The raw file contents. Raises
------
PromptLoadError - ``load_not_found``: key not found, traversal rejected, or missing target. - ``load_io``: OS error, metadata read failure, or ``max_bytes`` exceeded.

### `MemoryLoader`

```python
class MemoryLoader(prompts: 'dict[str, str] | None' = None) -> 'None'
```

Load prompt source from an in-memory key→text mapping. The primary use case is **dependency injection in tests**: production code uses a
:class:`FileSystemLoader` or a custom loader; tests substitute a :class:`MemoryLoader`
with hard-coded prompt text. No filesystem access is performed. Parameters
----------
prompts: A mapping of logical key → raw prompt text.
...
Return the mapped text for ``key``, or raise ``PromptLoadError(load_not_found)``. Parameters
----------
key: Logical identifier; matched exactly against the mapping keys. Raises
------
PromptLoadError ``load_not_found`` when ``key`` is absent from the mapping.

### `PromptLoadError`

```python
class PromptLoadError(*args, **kwargs)
```

A PromptLoader::load call failed. code = "load_not_found" when the key is absent; code = "load_io" on I/O error or exceeded max_bytes. Distinct from LoadError (which is the parse/shape error). Carries `.errors`: list[FieldError].

### `PromptLoader`

```python
class PromptLoader(*args, **kwargs)
```

Pluggable source of raw prompt text. A single ``load(key)`` operation maps a logical key to the raw text of a prompt
definition. The returned text is **never parsed or validated** — that belongs to the
construct-from-text path (``Prompt.from_yaml`` etc.). Any callable ``(key: str) -> str`` satisfies this protocol (FR-001 callable coercion):
no struct is required. Failures must raise :class:`~prompting_press.PromptLoadError` — never return empty
string or ``None`` for a missing key.
...
Return the raw text for ``key``, or raise :class:`~prompting_press.PromptLoadError`. Parameters
----------
key: A logical identifier for the prompt (e.g. ``"greet"``). Not a file path. Returns
-------
str The raw text content (YAML/JSON/TOML — callers decide the format). Raises
------
PromptLoadError ``load_not_found`` when the key is absent; ``load_io`` on I/O error or ``max_bytes`` exceeded.

### `make_prompt_load_error`

```python
def make_prompt_load_error(error_code, message)
```

Construct a Python [`PromptLoadError`] with a caller-supplied `[&#123;field, code, message&#125;]`
payload (FR-008a — native error-raise path for pure-Python loaders). Exposed as `prompting_press.make_prompt_load_error(code, message)` so pure-Python
