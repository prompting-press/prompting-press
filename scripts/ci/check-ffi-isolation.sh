...
for crate in "${COVERED_CRATES[@]}"; do
  for ffi in "${FFI_CRATES[@]}"; do
    # cargo tree -i <pkg> exits 0 if the package IS in the tree (bad);
    # exits non-zero with "did not match any packages" if absent (good).
    # cargo tree -i <pkg> exits 1 when the package is absent ("did not match any
    # packages") — which is the GOOD case. We capture output and use || true so
    # set -e does not abort on that expected non-zero exit.
