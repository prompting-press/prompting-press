...
- [ ] T002 Add the four `pub const` key strings (`gen_ai.prompt.name`, `.variant`, `.template_hash`, `.render_hash`) in the consumer crate `crates/prompting-press/src/` (render/result module).
- [ ] T003 Add a shared `provenance_attributes(&RenderResult) -> BTreeMap<String,String>` helper (or an `impl RenderResult` method) in the consumer building exactly the four entries from the existing fields. Pure: no I/O, no mutation (FR-004). Unit-test it (exactly 4 entries; values match; deterministic order).
...
**Independent Test**: render → helper → 4-entry `gen_ai.prompt.*` map whose values equal the result fields; passable to a span's bulk set-attributes.
...
- [ ] T007 [P] [US1] Rust consumer test: `result.provenance_attributes()` returns the 4 expected keys/values; default-variant render yields `gen_ai.prompt.variant == "default"` (INV-3).
...
- [ ] T012 [US2] Test that the helper is a pure projection: no I/O, mutates nothing, and render invokes no caller callback / has no side effect beyond returning the result (FR-004/FR-005). Assert the map excludes `text`/`guard`/metadata/`output_model` — exactly 4 keys (FR-007, SC-004).
...
## Phase 6: Constitution amendment (v3.0.0 — Principle V softening)

**⚠️ Governance (FR-012/013/014). Cites spec-017's repositioning statement.**

- [ ] T014 Amend `.specify/memory/constitution.md` Principle V: the library MAY **format** provenance into a flat attribute map (still data on the return value); still **no telemetry sink, no push/emit, no telemetry dependency**; the two hashes + semantics unchanged. **Cite the spec-017 v3.0.0 repositioning statement** (do not re-derive it). Update the version line / sync-impact note consistently with 017's v3.0.0 (coordinate: this is an additive Principle V softening under the same major line).
...
- [ ] T016 [P] Update `.specify/memory/roadmap.md`: add spec 018 entry (status) noting it cites the v3.0.0 repositioning.
...
- [ ] T019 [P] Docs: document `provenance_attributes()`/`provenanceAttributes()` in the docs site + each binding quickstart, incl. the fixed `gen_ai.prompt.*` keys and the honesty note (name/variant OTel-aligned; template_hash/render_hash are prompting-press extensions). Docs-are-product — current behavior only.
...
- Phase 6 (amendment) parallel with code but lands in the same change set; coordinate the v3.0.0 line with spec 017.
