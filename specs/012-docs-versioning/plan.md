...
### Render fidelity + root/next routing (2026-06-30 iteration — FR-019/FR-020, research R9/R10)

The dynamic versioned route is upgraded from a degraded raw-text page to full Starlight chrome, and the
canonical root is moved to the latest *released* version:

- **`StarlightPage` render**: `docs/site/src/pages/v/[version]/[...slug].astro` (and a new `/next/` route)
  render each versioned page through Starlight's `StarlightPage` component (`@astrojs/starlight/components`,
  present in the pinned 0.41.1) instead of `<pre>{bodyText}</pre>`. This requires the versioned MDX/Markdown
  to be *compiled* — via an Astro content collection over `src/versions/**` (and the working tree for
  `/next/`) or `import.meta.glob` — not read as raw text. Each route passes the entry's compiled body,
  frontmatter (title/description), and a manifest-derived `sidebar` prop (the autogenerated Starlight sidebar
  only covers the `docs` collection, so versioned routes supply their own).
- **Two full-Starlight roots**: the bare path `/` serves the latest *released* snapshot; `/next/…` serves
  the working `src/content/docs/` (main); older releases stay at `/vX.Y/…`. All three render with full chrome.
  The `latest`/`isLatest` selection stays auto-computed by `snapshot-docs.mjs` (no hardcoded version).
- **Old-version banner**: a shared component renders, on every non-latest page (`/next/` + pinned), a
  Starlight `<Aside>`: "You are looking at an old version; `<a href={latest canonical}>X.Y</a>` is the latest
  version" — label + href read from the manifest.
- **noindex + VersionSelect**: `/next/` and non-latest pinned pages carry `noindex`; the canonical `/` is
  indexable. `VersionSelect.astro` routes `isLatest → /`, `next → /next/`, pinned `→ /vX.Y/` (no longer
  collapsing `isLatest` and `next` onto the same bare URL — the shipped bug).
- **No new dependency / no migration / no plugin** — `StarlightPage` ships in the pinned Starlight; this
  stays inside spec 012's recorded "no third-party plugin / no platform migration" decisions.
